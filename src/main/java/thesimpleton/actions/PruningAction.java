...
public class PruningAction extends AbstractGameAction {
  ...
  public PruningAction(AbstractPlayer player, int numStacksToHarvest, int numStacksToGain, boolean isFromCard) {
    TheSimpletonMod.logger.info("============> PruningAction::constructor =====");
  ...
  public void update() {
    TheSimpletonMod.logger.info("============> PruningAction::update =====");

    Logger logger = TheSimpletonMod.logger;
    if (this.duration != ACTION_DURATION) {
      AbstractDungeon.actionManager.addToBottom(new WaitAction(0.1F));
      logger.info("PruningAction.update :: second tick");
      if (this.hasHarvested) {
        logger.info("PruningAction.update :: stacking " + this.oldestCropOrb.name + " for " + this.amount);
    ...
    if (!this.firstTickFinished) {
      ...
      if (AbstractCropOrb.playerHasAnyCropOrbs()) {
        ...
        logger.info("PruningAction.update :: harvesting " + this.oldestCropOrb.name + " for " + this.numStacksToHarvest);

        // this.oldestCropOrb.getCrop().harvest(false, this.numStacksToHarvest);
        TheSimpletonMod.logger.info("============> PruningAction::update queueing HarvestCropAction =====");

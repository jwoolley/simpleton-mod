...
public class FirefliesEvent extends AbstractImageEvent
{
  ...
  public FirefliesEvent() {
    ...
    if (player.masterDeck.getPowers().size() > 0 && !player.hasRelic(DEFAULT_REWARD_RELIC.relicId)) {
      relicReward = DEFAULT_REWARD_RELIC;
    } else if (player.masterDeck.getSkills().size() > 0 && !player.hasRelic(SECOND_REWARD_RELIC.relicId)) {
